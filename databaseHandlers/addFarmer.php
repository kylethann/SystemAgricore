...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($password == $cpassword) {
    ...
    $stmt->execute([$email]);
    if (!$stmt->rowCount() > 0) {
      $sql = $pdo->prepare("INSERT INTO `farmer1` (username, email, `password`, name, middleName, surname, gender, age, `address`, birthday, contactNumber, crops)
					VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
      $result = $sql->execute([$username, $email, $password, $name, $middleName, $surname, $gender, $age, $address, $birthday, $contactNumber, $crops]);
